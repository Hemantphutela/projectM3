...
server.post('/patients', function (request, res, next) {
  ...
  if (request.params.Patient.address === undefined ) {
    return next(new restify.InvalidArgumentError('Address must be supplied'))
  }

  // department is compulsory
  if (request.params.Patient.department === undefined ) {
    return next(new restify.InvalidArgumentError('department must be supplied'))
  }

  // doctor is compulsory
  if (request.params.Patient.doctor === undefined ) {
    return next(new restify.InvalidArgumentError('Doctor must be supplied'))
  }

  var newPatient = new Patients({
    first_name: request.body.Patient[0].first_name, 
    last_name: request.body.Patient[0].last_name,
    dob: request.body.Patient[0].dob,
    address: request.body.Patient[0].address,
    department: request.body.Patient[0].department,
    doctor: request.body.Patient[0].doctor
  });
